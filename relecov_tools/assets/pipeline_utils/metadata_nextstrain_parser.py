...
def parse_args(args=None):
    ...
    parser.add_argument("--input-dir", required=True, help="Directory containing input JSON files (each containing an array of records)")
    return parser.parse_args(args)

def extract_strain_from_filepath(filepath):
    """Extract strain name from consensus_sequence_filepath"""
    if not filepath or filepath == "Not Provided [SNOMED:434941000124101]" or filepath.startswith("Not Provided"):
        return "unknown"
    ...
    try:
        ...
        for record in data:
            ...
            tsv_record = {
                "strain": strain,
                "virus": "ncov",
                "gisaid_epi_isl": clean_gisaid_epi_isl(record.get("gisaid_accession_id", "NA")),
                ...
                "originating_lab": clean_value(record.get("collecting_institution", "NA")),
                "submitting_lab": clean_value(record.get("submitting_institution", "NA")),
        ...
        if influenza_count > 0:
            print(f"Processed {json_file}: {len(records)} records, {sequences_written} sequences written ({influenza_count} influenza samples excluded)")
        else:
            print(f"Processed {json_file}: {len(records)} records, {sequences_written} sequences written")
...
def main(args=None):
    ...
    if not json_files:
        print(f"No JSON files starting with 'bioinfo_lab_metadata' found in {args.input_dir}")
        print(f"Available files: {os.listdir(args.input_dir)}")
        sys.exit(1)
    
    print(f"Found {len(json_files)} JSON files starting with 'bioinfo_lab_metadata' to process")
    
    # Define the header - removed genbank_accession and url columns
    header = [
        "strain", "virus", "gisaid_epi_isl", "pango_lineage", "date", "region", 
        "country", "division", "location", "region_exposure", "country_exposure", 
        "division_exposure", "segment", "length", "host", "age", "sex", 
        "originating_lab", "submitting_lab", "authors", "title"
    ]
            ...
            try:
                ...
                if isinstance(data, list):
                    for record in data:
                        ...
                        elif extract_strain_from_filepath(record.get("consensus_sequence_filepath", "")) == "unknown":
    ...
    if total_no_consensus_excluded > 0:
        print(f"Samples without consensus filepath excluded: {total_no_consensus_excluded}")
    ...
    try:
        ...
        print(f"✓ {os.path.basename(sequences_output)} - Concatenated consensus sequences")
