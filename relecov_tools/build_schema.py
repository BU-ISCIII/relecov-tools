...
class SchemaBuilder:
    def __init__(
        ...
        if base_schema_path is not None:
            if relecov_tools.utils.file_exists(base_schema_path):
                ...
                log.error(f"[Error]Defined base schema file not found: {base_schema_path}.")
                stderr.print(
            ...
            try:
                ...
                try:
                    ...
                    if not relecov_tools.utils.file_exists(self.base_schema_path):
                        log.error("[Error]Fatal error. Relecov schema were not found in current relecov-tools installation. Make sure relecov-tools command is functioning.")
                        stderr.print(
    ...
    def read_database_definition(self, sheet_id="main"):
        ...
        if validation_out:
            log.error(f"({caller_method}:{sheet_id}) Validation of database content falied. Missing mandatory features in: {validation_out}")
            stderr.print(
                f"({caller_method}:{sheet_id}) [red]Validation of database content falied. Missing mandatory features in: {validation_out}"
            )
            sys.exit(1)
        else:
            log.info(f"({caller_method}:{sheet_id}) Validation of database content passed.")
            stderr.print(
                ...
                # For those that follows standard format, add them to json schema as well.
                    ...
                    for db_feature_key, schema_feature_key in features_to_check.items():
                        ...
                        if db_feature_key not in db_features_dic:
                            log.info(f"Feature {db_feature_key} is not present in database ({self.excel_file_path})")
                            stderr.print(
    ...
    def get_schema_diff(self, base_schema, new_schema):
        ...
        if not diff_lines:
            log.info("No differencess were found between already installed and new generated schema. Exiting. No changes made")
            stderr.print(
                "[yellow]No differencess were found between already installed and new generated schema. Exiting. No changes made"
            )
            return None
        else:
            log.info("Differences found between the existing schema and the newly generated schema.")
            stderr.print(
