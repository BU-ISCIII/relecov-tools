...
class BioinfoMetadata:
    ...
    def include_pangolin_data(self, j_data):
        ...
        """
        mapping_fields = self.configuration.get_topic_data("bioinfo_analysis", "mapping_pangolin")
        for row in j_data:
    ...
    def include_variant_metrics(self, j_data):
        ...
        mapping_fields = self.configuration.get_topic_data("bioinfo_analysis", "mapping_variant_metrics")
        for row in j_data:
            for field, value in mapping_fields.items():
                try:
                    row[field] = map_data[row["sample_name"]][value]
    ...
    def create_bioinfo_file(self):
        ...
        file_name = (
            "bioinfo_"
            + os.path.splitext(os.path.basename(self.json_file))[0]
            + ".json"
        )
